...
targetFiles.forEach(file => {
  ...
  const startMarker = '<button type="button" id="p1-btn-plans"';
  const endMarker = '<!-- Micro Footer de Certificación';

  const startIdx = html.indexOf(startMarker);
  const endIdx = html.indexOf(endMarker, startIdx);

  if (startIdx !== -1 && endIdx !== -1) {
    // Find the beginning of the button container before p1-btn-plans
    const containerStart = html.lastIndexOf('<div style="display:flex; flex-wrap:wrap;', startIdx);

    const cleanThreeButtons = `<div style="display:flex; flex-wrap:wrap; justify-content:center; gap:16px; margin-bottom:24px;">
      ...
      <button type="button" id="p1-btn-ceo-key" data-test-id="p1-btn-master" onclick="abrirAccesoClaveCEO()" style="padding:16px 28px; background:rgba(255,199,44,0.12); color:#FFC72C; border:1.8px solid #FFC72C; border-radius:14px; font-family:'Montserrat', sans-serif; font-weight:900; font-size:13.5px; cursor:pointer; text-transform:uppercase; letter-spacing:0.8px; box-shadow:0 8px 30px rgba(255,199,44,0.25); display:inline-flex; align-items:center; gap:10px; transition:all 0.25s ease;" onmouseover="this.style.background='rgba(255,199,44,0.25)'; this.style.transform='translateY(-3px) scale(1.02)'; this.style.boxShadow='0 12px 40px rgba(255,199,44,0.5)';" onmouseout="this.style.background='rgba(255,199,44,0.12)'; this.style.transform='translateY(0) scale(1)'; this.style.boxShadow='0 8px 30px rgba(255,199,44,0.25)';">
        <span style="font-size:18px; filter:drop-shadow(0 2px 6px rgba(0,0,0,0.6));">👑</span>
        <span id="p1-btn-master-text">ACCESO MASTER</span>
      </button>
    ...
    html = html.substring(0, containerStart) + cleanThreeButtons + html.substring(endIdx);
    fs.writeFileSync(file, html, 'utf8');
    console.log(`Successfully updated ${file} with strictly the 3 buttons`);
  } else {
